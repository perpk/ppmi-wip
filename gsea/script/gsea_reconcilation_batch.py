...
def consolidate_ml_and_write_to_csv(source_dir, filename_prefix, output_dir, age_groups, genders, visits, classifiers):
    for classifier in classifiers:
        for gender in genders:
            for age_group in age_groups:
                ...
                gsea_dfs_raw = {}
                for visit in visits:
                    gsea_file = Path(source_dir) / f"{classifier}_{filename_prefix}_{gender}_{visit}_{age_group}.csv"
                ...
                rows_with_common_terms.to_csv(f"{output_dir}/enr_results_sorted_common_terms_{classifier}_{gender}_{age_group}.csv")
...
def main():
    ...
    classifiers = ["LR2"]#["LR2", "SVM2", "RF2", "XGBOOST2"]
    ...
    consolidate_ml_and_write_to_csv(GSEA_PATH_ML, "enr_ml_results_sorted", GSEA_RESULTS_PATH_ML, age_groups, genders, visits, classifiers)
