...
PATH: Final = "/Users/kpax/Documents/aep/study/MSC/lab/PPMI_Project_133_RNASeq/data/ml/classification/XGBOOST/deg_classification/"
    ...
    try:
        with warnings.catch_warnings():
            ...
            grid_search = GridSearchCV(estimator=xgboost_pipeline,
                                       ...
                                       scoring='roc_auc',
                                       cv=StratifiedKFold(10, shuffle=True, random_state=42),  # Fewer folds for speed
                                       n_jobs=-1,
                                       verbose=2)
...
def main():
    ...
    for gender in genders:
        for age_group in age_groups:
            ...
            degs = pd.read_csv(DEG_SOURCE_PATH + f"DEGs_stratified_consoVisits_{gender}_{age_group}.csv", index_col=0)
            ...
            result = train_xgboost(ppmi_ad_subset, f"{gender}_{age_group}")
            if result is None:
                print(f"Failed to train XGBoost for {age_group} - skipping")
                continue
